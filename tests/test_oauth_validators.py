...
class TestGoogleOAuthValidator:
    ...
    # ИСПРАВЛЕННЫЕ ТЕСТЫ для обработки исключений

    @patch("requests.get")
    def test_get_token_info_network_error(self, mock_get, validator):
        """Test network error during token info retrieval - ИСПРАВЛЕНО"""
        mock_get.side_effect = Exception("Network error")

        # Метод должен обрабатывать исключение и возвращать None
    ...
    def test_get_user_info_network_error(self, mock_get, validator):
        """Test network error during user info retrieval - ИСПРАВЛЕНО"""
        mock_get.side_effect = Exception("Network error")
    ...
    def test_get_user_info_json_decode_error(self, mock_get, validator):
        """Test JSON decode error handling - ИСПРАВЛЕНО"""
        mock_response = Mock()
    ...
    def test_process_user_info_missing_required_fields(self, validator):
        """Test processing user info with missing required fields - ИСПРАВЛЕНО"""
        raw_info = {}  # Empty info

        result = validator._process_user_info(raw_info)

        # Должно корректно обрабатывать отсутствующие поля
        assert result["google_id"] == ""  # ИСПРАВЛЕНО: get() возвращает "" для пустого словаря
        assert result["email"] == ""
