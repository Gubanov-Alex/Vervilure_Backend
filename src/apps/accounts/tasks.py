    ...
    try:
        ...
        while attempt < max_db_attempts and not user:
            try:
                ...
                logger.error(
                    f"Database error on attempt {attempt}: {db_error}", extra={"user_id": user_id, "email": email}
                )
        ...
        if not user:
            ...
            logger.error(f"User not found after {max_db_attempts} attempts: ID={user_id}, email={email}")
        ...
        logger.error(
            f"Failed to send verification email: {exc}", extra={"user_id": user_id, "email": email}, exc_info=True
        )
        ...
        try:
            ...
            logger.error(f"Max retries exceeded for verification email", extra={"user_id": user_id, "email": email})
            return None
...
def send_password_reset_email(self, user_id: int, reset_token: str) -> Optional[str]:
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Max retries exceeded for password reset email", extra={"user_id": user_id})
            return None
