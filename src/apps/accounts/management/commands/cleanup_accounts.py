...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"Starting account cleanup process..."))

        # Find expired deactivated accounts
        cutoff_date = timezone.now() - timedelta(days=days)
        expired_users = User.objects.filter(
            deactivated_at__lt=cutoff_date, is_anonymized=False, is_active=False
        ).select_related()

        count = expired_users.count()

        if count == 0:
            self.stdout.write(self.style.WARNING("No expired deactivated accounts found."))
            return

        self.stdout.write(self.style.WARNING(f"Found {count} expired deactivated accounts."))

        if dry_run:
            self.stdout.write(self.style.SUCCESS("DRY RUN - No changes will be made:"))
            for user in expired_users:
                self.stdout.write(f"  - {user.email} (deactivated: {user.deactivated_at})")
            return

        # Confirmation prompt
        if not force:
            confirm = input(f"This will anonymize {count} accounts. " "Are you sure you want to continue? [y/N]: ")
            if confirm.lower() != "y":
        ...
        for user in expired_users:
            try:
                ...
                anonymized_count += 1
                self.stdout.write(self.style.SUCCESS(f"Anonymized: {original_email} -> {anonymous_id}"))
                ...
                logger.error(
                    ...
                    extra={"user_id": user.id, "error": str(e), "action": "auto_account_anonymize_error"},
                    exc_info=True,
                )
    ...
    def _cleanup_related_data(self):
        ...
        if deleted_tokens > 0:
            self.stdout.write(self.style.SUCCESS(f"Cleaned up {deleted_tokens} expired blacklisted tokens."))
